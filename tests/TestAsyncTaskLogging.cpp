...
void TestAsyncTaskLogging::testConcurrentTasksIsolation()
{
    ...
    for (int i = 0; i < taskCount; ++i) {
        ...
        QString html = logVm->data(idx, LogViewModel::FormattedMessagesRole).toString();
        // Verify NO logs from other tasks leaked into this task's text
        for (int other = 0; other < taskCount; ++other) {
            if (other != i) {
                QVERIFY(!html.contains(QStringLiteral("Task %1 entry").arg(other + 1)));
            }
        }
    }
...
void TestAsyncTaskLogging::testCopyAllFormat()
{
    ...
    // Wait for logVm to process
    logVm.writeBlock(task->sealedBlocks().first(), task->taskName());
