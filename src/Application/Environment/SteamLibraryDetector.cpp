    ...
    std::shared_ptr<Core::Logging::TaskLoggingContext> logCtx)
{
    return Application::Execution::ExecutionGuard::guard<std::vector<SteamLibrary>>([&]() -> Core::Result<std::vector<SteamLibrary>> {
        ...
        if (vdfPath.exists() && vdfPath.isFile()) {
            ...
            // Case B: Valid VDF but genuinely 0 library entries defined
            if (logCtx) {
                logCtx->debug(QStringLiteral("libraryfolders.vdf is valid but contains no library entries: %1").arg(vdfPath.toString()));
            }
            return Core::Result<std::vector<SteamLibrary>>::success({});
        }

        // Case C: libraryfolders.vdf does not exist at all -> fallback to single Steam root library
        if (logCtx) {
...
Core::Result<QString> SteamLibraryDetector::readAppInstallDir(const Core::Path::FilesystemPath& libraryPath, int appId) {
...
Core::Result<QString> SteamLibraryDetector::readAppName(const Core::Path::FilesystemPath& libraryPath, int appId) {
    return Application::Execution::ExecutionGuard::guard<QString>([&]() -> Core::Result<QString> {
        if (!libraryPath.isValid() || !libraryPath.isDirectory()) {
            return Core::Result<QString>::failure(
                Core::Error::ErrorCode::DirectoryNotFound,
